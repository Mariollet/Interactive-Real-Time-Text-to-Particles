...
<head>
	...
	<link rel="icon" type="image/svg+xml" href="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 64 64'%3E%3Cg transform='rotate(180 32 32)'%3E%3Ccircle fill='%23FFD600' cx='32' cy='32' r='30'/%3E%3Cellipse fill='%23000' cx='22' cy='26' rx='4' ry='6'/%3E%3Cellipse fill='%23000' cx='42' cy='26' rx='4' ry='6'/%3E%3Cpath d='M20 44 Q32 54 44 44' stroke='%23000' stroke-width='4' fill='none' stroke-linecap='round'/%3E%3C/g%3E%3C/svg%3E">
...
<body>
	...
	<!-- UI: Font and parameters menu (Bootstrap card) -->
	<div id="fontMenu" class="card p-2">
		<div class="card-body text-white">
			...
			<!-- Sentence input -->
			<div class="mb-3">
				<label for="sentenceInput" class="form-label">TEXT: </label>
				<input type="textArea" id="sentenceInput" class="form-control form-control-sm bg-transparent text-white" value="CASSEZ\nLES CODES"
					autocomplete="off">
			</div>

			<hr class="my-2">

			<!-- Group: Fluidity parameters -->
			<div class="mt-3">
				<div>
					<label for="drag" class="form-label">DRAG: <span id="dragValue"></span></label>
					<input type="range" class="form-range" id="drag" min="0.8" max="1" step="0.01" value="0.99">
				</div>
				<div>
			...
			<div class="mt-3">
				...
				<div>
					...
					<input type="range" class="form-range" id="margin" min="0" max="300" step="25" value="100">
				</div>
				<div>
					<label for="thickness" class="form-label">THICKNESS: <span id="thicknessValue"></span></label>
					<input type="range" class="form-range" id="thickness" min="50" max="200" step="5" value="120">
				</div>
			</div>
		</div>
	</div>

	<!-- GitHub repo link, fixed at bottom right -->
	<a href="https://github.com/Mariollet/interactive_particles_typography" target="_blank" id="github-link" class="position-fixed bottom-0 end-0 m-3 text-decoration-none text-secondary small d-flex align-items-center"
		style="z-index:999;">
	...
	<script>
		...
		document.addEventListener('DOMContentLoaded', function() {
			optionIds.forEach(opt => {
				...
				document.getElementById(opt.id).addEventListener('input', function(e) {
					...
					// Smooth transition for structure-changing options
		...
		// --- FONT MENU: Change font with smooth transition ---
		...
		// Update sentence on input (with live transition)
		document.addEventListener('DOMContentLoaded', function() {
			const sentenceInput = document.getElementById('sentenceInput');
			if (sentenceInput) {
				sentenceInput.addEventListener('input', function(e) {
					currentSentence = e.target.value || " ";
					init(true);
				});
				// Support \n for multiline
				sentenceInput.addEventListener('change', function(e) {
					currentSentence = e.target.value || " ";
					init(true);
				});
			}
		...
		// --- APP START: Wait for fonts, then launch animation ---
